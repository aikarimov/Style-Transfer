...
FName = {'VanGogh_Chestnut','Seurat_Bridge','Levitan_Oak','VanGogh_Wheat','Seurat_Chahut','Levitan_Evening','VanGogh_Lenna_Iris','Seurat_Lenna_Bridge','Levitan_Lenna_Oak'};
...
Name = {'VanGogh Chestnut','Seurat Bridge','Levitan Oak','VanGogh Wheat','Seurat Le Chahut','Levitan Village','VanGogh Lenna (Iris)','Seurat Lenna (Bridge)','Levitan Lenna (Oak)'};
nfiles = length(Name);
nx = 3; ny = 3; %supltols
fsize = 10; %font size
...
markers = {'^','square','o','^','square','o','^','square','o'};
...
for n=1:nfiles
    ...
    set(gcf,'position',[30   50  800  600]);
    subplot(nx,ny,n);
...
for i = 1:6
    ...
    switch i
        case 1
            cdata = abs(corrcoef(clength));
            str = 'length histogram correlation';
        case 2
            cdata = abs(corrcoef(cstraightness));
            str = 'straightness histogram correlation';
        case 3
            cdata = abs(corrcoef(corientation));
            str = 'orientation histogram correlation';
        case 4
            cdata = abs(corrcoef(cnbsnb));
            str = 'orientation histogram correlation';
        case 5
            cdata = abs(corrcoef(cnbsso));
            str = 'NBS-NB histogram correlation';
        case 6
            cdata = abs(corrcoef(cosdnb));
            str = 'OSD-NB histogram correlation';
    end
    ii = ones(size(cdata)); %make lower triangular matrix
    idx = tril(ii);
    cdata(~idx) = nan;
    h = heatmap(Name,Name,cdata, 'MissingDataColor', 'w','GridVisible', 'off', 'MissingDataLabel', " ");
    h.Title = str;
    set(gcf,'position',[360  200  630  415]);
    colormap turbo
    colorbar off
end
